...
async function main() {
  ...
  if (!tenant) {
    tenant = await prisma.tenant.create({
      data: {
        name: "Oficina Demo — Eletromecânica",
        slug: "demo",
  ...
  if (serviceCount === 0) {
    await prisma.serviceCatalog.createMany({
      data: [
        ...
        {
          tenantId: tenant.id,
          name: "Diagnóstico OBD / scanner",
          category: "Injeção eletrônica / diagnóstico",
          price: 150,
          durationMin: 45,
          warrantyDays: 0,
        },
        {
          tenantId: tenant.id,
          name: "Recarga e teste de bateria",
          category: "Bateria",
          price: 80,
          durationMin: 30,
          warrantyDays: 30,
